...
func TestMorningOrEveningStar(t *testing.T) {
	ts := []types.KLine{
		...
		{Open: n(20.30), Low: n(19.60), High: n(20.45), Close: n(19.60)},
		{Open: n(20.70), Low: n(20.40), High: n(21.82), Close: n(20.40)},
	}
	stream := &types.StandardStream{}
	kLines := v2.KLines(stream, "", "")
	ind := MorningOrEveningStar(kLines)
	...
	if ind.Last(0) != expectedBear {
		t.Errorf("TestMorningOrEveningStar Bear unexpected result: got %v want %v", ind.Last(0), expectedBear)
	}
	...
	if ind.Last(0) != expectedBull {
		t.Errorf("TestMorningOrEveningStar Bull unexpected result: got %v want %v", ind.Last(0), expectedBull)
	}
}
