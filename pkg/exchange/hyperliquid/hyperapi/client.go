...
func NewClient() *Client {
	...
	return &Client{
		...
		nonce: nonce.NewSecondNonce(time.Now()),
	}
}
...
func (c *Client) convertToAction(action any) ([]byte, error) {
	...
	// Convert actionType to ReqTypeInfo for comparison
	// Support both string and ReqTypeInfo types
	var reqType ReqTypeInfo
	switch v := actionType.(type) {
	case string:
		reqType = ReqTypeInfo(v)
	case ReqTypeInfo:
		reqType = v
	default:
		return nil, fmt.Errorf("action type must be a string or ReqTypeInfo, got %T", actionType)
	}
