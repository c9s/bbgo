...
func TestSignatureAddressRecovery(t *testing.T) {
	...
	// Rebuild the EIP-712 hash (same as in sign method)
	...
	hashData := []byte{0x19, 0x01}
	hashData = append(hashData, domainSeparator...)
	hashData = append(hashData, typedDataHash...)
	hash := crypto.Keccak256(hashData)

	// Recover the address from signature
	// hexutil.EncodeBig returns hex string with "0x" prefix
	rHex := signature.R
	if len(rHex) > 0 && rHex[:2] != "0x" {
		rHex = "0x" + rHex
	}
	rBytes, err := hexutil.Decode(rHex)
	if err != nil {
		t.Fatalf("Failed to decode R: %v", err)
	}

	sHex := signature.S
	if len(sHex) > 0 && sHex[:2] != "0x" {
		sHex = "0x" + sHex
	}
	sBytes, err := hexutil.Decode(sHex)
	if err != nil {
	...
	sigBytes[64] = byte(signature.V - 27) // Convert V from [27,28] to [0,1]

	// Recover public key from signature
	recoveredPubKey, err := crypto.Ecrecover(hash, sigBytes)
	if err != nil {
