...
func (f *DepthFrame) loadFuturesDepthSnapshot() error {
	if debugBinanceDepth {
		log.Infof("buffering %s depth events...", f.Symbol)
	}
	...
	depth, err := f.futuresFetch(f.context)
	if err != nil {
		return err
	}
...
// fetch fetches the depth and convert to the depth event so that we can reuse the event structure to convert it to the global orderbook type
func (f *DepthFrame) futuresFetch(ctx context.Context) (*DepthEvent, error) {
	if debugBinanceDepth {
		log.Infof("fetching %s depth snapshot", f.Symbol)
	}

	response, err := f.futuresClient.NewDepthService().Symbol(f.Symbol).Do(ctx)
	if err != nil {
		return nil, err
	}
