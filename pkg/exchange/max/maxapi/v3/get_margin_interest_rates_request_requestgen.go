...
func (g *GetMarginInterestRatesRequest) applySlugsToUrl(url string, slugs map[string]string) string {
	for _k, _v := range slugs {
		...
		if cached, ok := GetMarginInterestRatesRequestSlugReCache.Load(_k); ok {
			needleRE = cached.(*regexp.Regexp)
		} else {
			needleRE = regexp.MustCompile(":" + _k + "\\b")
			GetMarginInterestRatesRequestSlugReCache.Store(_k, needleRE)
		}
...
func (g *GetMarginInterestRatesRequest) GetPath() string {
	return "/api/v3/wallet/m/interest_rates"
}
