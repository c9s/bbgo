...
func down_main_addIsFuturesColumn(ctx context.Context, tx rockhopper.SQLExecutor) (err error) {
	...
	_, err = tx.ExecContext(ctx, "ALTER TABLE trades DROP COLUMN IF EXISTS is_futures;")
	if err != nil {
		return err
	}
	_, err = tx.ExecContext(ctx, "ALTER TABLE orders DROP COLUMN IF EXISTS is_futures;")
	if err != nil {
		return err
	}
	return err
