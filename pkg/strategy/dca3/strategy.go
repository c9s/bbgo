...
type Strategy struct {
	...
	lastUpdateTakeProfitOrderAt time.Time
...
func (s *Strategy) Run(ctx context.Context, _ bbgo.OrderExecutor, session *bbgo.ExchangeSession) error {
	...
	/*
		s.OrderExecutor.TradeCollector().OnPositionUpdate(func(position *types.Position) {
			s.logger.Infof("POSITION UPDATE: %s", position.String())

			if s.stateMachine.GetState() == StateOpenPositionReady && s.Position.GetBase().Abs().Compare(s.Market.MinQuantity) >= 0 {
				s.logger.Infof("state is StateOpenPositionReady position base (%s) is greater than or equal to min quantity (%s), emit next state", s.Position.GetBase().String(), s.Market.MinQuantity.String())
	...
	*/
	...
	s.OrderExecutor.TradeCollector().OnTrade(func(trade types.Trade, profit, netProfit fixedpoint.Value) {
		s.logger.Infof("trade received: %s", trade.String())
