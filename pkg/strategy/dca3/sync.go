...
func (s *Strategy) syncPeriodically(ctx context.Context, validateStateC chan struct{}) {
	...
	for {
		...
		case <-validateStateTicker.C:
			s.logger.Infof("validating state... (current state: %d)", s.stateMachine.GetState())
			s.validateState(ctx)
		case <-validateStateC:
			s.logger.Infof("validating state triggered... (current state: %d)", s.stateMachine.GetState())
			s.validateState(ctx)
		}
	}
}
