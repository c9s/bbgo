...
func TestProfitStats_NewRound(t *testing.T) {
	...
	assert.Equal(t, fixedpoint.NewFromFloat(1.02), stats.TotalFee["USDT"])
	...
	// 檢查當前輪次獲利
	assert.Equal(t, fixedpoint.NewFromFloat(-992), stats.CurrentRoundProfit)

	// 檢查總獲利更新
	expectedTotalProfit := firstRoundTotalProfit.Add(fixedpoint.NewFromFloat(-992))
	assert.Equal(t, expectedTotalProfit, stats.TotalProfit)

	// 檢查手續費累計
	assert.Equal(t, fixedpoint.NewFromFloat(0.0008), stats.CurrentRoundFee["BTC"])
