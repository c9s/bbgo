...
var stateTransition map[State]State = map[State]State{
	...
	OpenPositionOrderFilled: OpenPositionFinish,
	OpenPositionFinish:      TakeProfitReady,
...
// it wwill not trigger the next state immediately because OpenPositionOrderFilled state only trigger by kline to move to the next state
...
func (s *Strategy) finishOpenPositionStage(_ context.Context) bool {
	s.updateState(OpenPositionFinish)
	s.logger.Info("[State] OpenPositionOrderFilled -> OpenPositionFinish")
	return true
}
...
func (s *Strategy) finishTakeProfitStage(ctx context.Context, next State) bool {
	...
	s.logger.Info("[State] finishTakeProfitStage - start reseting position and calculate quote investment for next round")
