...
func recoverStateIfAtTakeProfitStage(orders []types.Order, activeOrderBook *bbgo.ActiveOrderBook, orderStore *core.OrderStore) (State, error) {
	...
	openedOrders, cancelledOrders, filledOrders, unexpectedOrders := types.ClassifyOrdersByStatus(orders)
...
func recoverStateIfAtOpenPositionStage(orders []types.Order, activeOrderBook *bbgo.ActiveOrderBook, orderStore *core.OrderStore) (State, error) {
	openedOrders, cancelledOrders, filledOrders, unexpectedOrders := types.ClassifyOrdersByStatus(orders)
	if len(unexpectedOrders) > 0 {
