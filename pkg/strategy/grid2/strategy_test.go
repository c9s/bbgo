...
func Test_verifyFilledGrid(t *testing.T) {
	...
	t.Run("valid grid with buy/sell orders", func(t *testing.T) {
		pinOrderMap := PinOrderMap{
			Pin(number("100.00")): types.Order{
				OrderID: 1,
				SubmitOrder: types.SubmitOrder{
					Side: types.SideTypeBuy,
				},
			},
			Pin(number("200.00")): types.Order{},
			Pin(number("300.00")): types.Order{
				OrderID: 3,
				SubmitOrder: types.SubmitOrder{
					Side: types.SideTypeSell,
				},
			},
			Pin(number("400.00")): types.Order{
				OrderID: 4,
				SubmitOrder: types.SubmitOrder{
					Side: types.SideTypeSell,
				},
