...
type Strategy struct {
	*common.Strategy
	...
	// EMA 技術指標設定
	EMAWindow types.IntervalWindow `json:"emaWindow"` // EMA視窗設定，可自訂天數與週期
...
func (s *Strategy) Defaults() error {
	...
	// 預設使用 120 倍 volume interval
	if s.EMAWindow == (types.IntervalWindow{}) {
		s.EMAWindow = types.IntervalWindow{
			Interval: s.VolumeInterval,
			Window:   120,
		}
	}
	return nil
...
func (s *Strategy) Subscribe(session *bbgo.ExchangeSession) {
	...
	// 訂閱EMA指標所需的K線數據
	session.Subscribe(types.KLineChannel, s.Symbol, types.SubscribeOptions{Interval: s.EMAWindow.Interval})
...
func (s *Strategy) checkEMAFilter(closePrice fixedpoint.Value) bool {
	...
	emaValue := fixedpoint.NewFromFloat(s.ema.Last(0))
	// 如果價格從EMA上方跌破到下方，跳過進場
	if closePrice.Compare(emaValue) < 0 {
		log.Infof("Price %s is below EMA %s, skipping entry", closePrice, emaValue)
		return false
	}
...
func (s *Strategy) Run(ctx context.Context, orderExecutor bbgo.OrderExecutor, session *bbgo.ExchangeSession) error {
	...
	s.ema = standardIndicatorSet.EWMA(s.EMAWindow)
	...
	session.MarketDataStream.OnKLineClosed(func(kline types.KLine) {
		...
		log.Infof("[Entry] Checking if closed price %f is above EMA %f", closePrice.Float64(), s.ema.Last(0))
