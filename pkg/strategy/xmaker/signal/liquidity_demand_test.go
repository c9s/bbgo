...
// createGreenKLine creates a green (bullish) candle with high buy demand
// Buy demand is concentrated (small range above open) indicating strong buying pressure
func createGreenKLine(open, close float64, t time.Time) types.KLine {
...
// createRedKLine creates a red (bearish) candle with high sell demand
// Sell demand is concentrated (small range below open) indicating strong selling pressure
func createRedKLine(open, close float64, t time.Time) types.KLine {
...
func newTestLiquidityDemandSignal() (*LiquidityDemandSignal, *indicatorv2.KLineStream) {
	...
	signal := &LiquidityDemandSignal{
		IntervalWindow:         iw,
		Threshold:              100.0, // Lower threshold for testing (adjusted to match test data)
...
func TestLiquidityDemandSignal_SixConsecutiveGreenCandles(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		close := open + 4.0 // Green candle (close > open)
		// For increasing buy demand: progressively smaller upward wick, progressively larger downward wick
		highAboveOpen := 1.0 - float64(i)*0.1 // Decreasing range above open (more concentrated buying over time)
		lowBelowOpen := 3.0 + float64(i)*0.5  // Increasing range below open (more distributed selling over time)
		kline := types.KLine{
			Open:      fixedpoint.NewFromFloat(open),
			Close:     fixedpoint.NewFromFloat(close),
			High:      fixedpoint.NewFromFloat(open + highAboveOpen),
			Low:       fixedpoint.NewFromFloat(open - lowBelowOpen),
			Volume:    fixedpoint.NewFromFloat(10000.0), // Constant volume to isolate price range effect
...
func TestLiquidityDemandSignal_SixConsecutiveRedCandles(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		open := basePrice - float64(i)*5.0
		close := open - 4.0 // Red candle (close < open)
		// For increasing sell demand: progressively larger upward wick, progressively smaller downward wick
		highAboveOpen := 3.0 + float64(i)*0.5 // Increasing range above open (more distributed buying over time)
		lowBelowOpen := 1.0 - float64(i)*0.1  // Decreasing range below open (more concentrated selling over time)
		kline := types.KLine{
			Open:      fixedpoint.NewFromFloat(open),
			Close:     fixedpoint.NewFromFloat(close),
			High:      fixedpoint.NewFromFloat(open + highAboveOpen),
...
func TestLiquidityDemandSignal_ThreeGreenThenThreeRed(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		kline := createGreenKLine(open, close, baseTime.Add(time.Duration(i)*time.Minute))
		klines = append(klines, kline)
	}
	...
	for i := 3; i < 6; i++ {
		...
		kline := createRedKLine(open, close, baseTime.Add(time.Duration(i)*time.Minute))
		klines = append(klines, kline)
	}
...
func TestLiquidityDemandSignal_WarmupPeriod(t *testing.T) {
	...
	baseTime := time.Now()
	klines := []types.KLine{
		createGreenKLine(100.0, 104.0, baseTime),
		createGreenKLine(105.0, 109.0, baseTime.Add(time.Minute)),
	}
