...
func (environ *Environment) syncWithUserConfig(ctx context.Context, userConfig *Config) error {
	...
	if err := environ.verifyRecords(ctx, userConfig); err != nil {
...
func (environ *Environment) verifyRecords(ctx context.Context, userConfig *Config) error {
	...
	if len(verifyCfg.Sessions) > 0 {
		sessions = environ.SelectSessions(verifyCfg.Sessions...)
	}
	...
	for _, session := range sessions {
		...
		for _, symbol := range symbols {
			// verify orders
			orphanOrders, err := environ.verifyOrdersForSymbol(ctx, api, session.Exchange.Name(), symbol, since, now)
			if err != nil {
				...
				case VerifyActionWarn:
					for _, o := range orphanOrders {
						log.Warnf("orphan order detected: %s", o)
					}
				case VerifyActionDelete:
					gids := make([]uint64, 0, len(orphanOrders))
					for _, o := range orphanOrders {
						gids = append(gids, o.GID)
						log.Warnf("deleting orphan order: %s", o)
					}
					if err := environ.OrderService.DeleteByGID(ctx, gids); err != nil {
						log.WithError(err).Errorf("failed to delete orphan orders for %s", symbol)
					}
				}
			} else {
			...
			// verify trades
			orphanTrades, err := environ.verifyTradesForSymbol(ctx, api, session.Exchange.Name(), symbol, since, now)
			if err != nil {
				...
				case VerifyActionDelete:
					...
					if err := environ.TradeService.DeleteByGID(ctx, gids); err != nil {
...
func (environ *Environment) verifyOrdersForSymbol(
	ctx context.Context,
	...
	aggOrders, err := environ.OrderService.Query(service.QueryOrdersOptions{
...
func (environ *Environment) verifyTradesForSymbol(
	ctx context.Context,
	api types.ExchangeTradeHistoryService,
	exchangeName types.ExchangeName,
	symbol string,
	...
	localTrades, err := environ.TradeService.Query(service.QueryTradesOptions{
		Exchange: exchangeName,
		Symbol:   symbol,
		Since:    &since,
		Until:    &until,
